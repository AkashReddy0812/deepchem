...
class DeepPROTAC(TorchModel):
    def __init__(self, **kwargs):
        ...
        ligase_ligand = GraphConv(num_embeddings=10)  # Replace 10 with the actual number of embeddings
        ligase_pocket = GraphConv(num_embeddings=5)   # Replace 5 with the actual number of embeddings
        target_ligand = GraphConv(num_embeddings=10)  # Replace 10 with the actual number of embeddings
        target_pocket = GraphConv(num_embeddings=5)   # Replace 5 with the actual number of embeddings
        smiles = SmilesNet(batch_size=1)              # Set batch_size as needed

        # Initialize the ProtacModel
        model = ProtacModel(
